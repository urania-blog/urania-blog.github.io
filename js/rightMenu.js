let rightMenu = {};
rightMenu.showRightMenu = function (isTrue, x = 0, y = 0) {
...
rightMenu.reloadrmSize = function () {
...
$(document).unbind('keydown').bind('keydown', function(e){
	...
	if(e.ctrlKey && e.keyCode  == 67) {
		const content = window.getSelection().toString(); //获取复制的内容
		if(!content) {
			toast("未勾选任何内容",2000)
			return false;
		}
...
function messageBox(html, title, message, type) {
    var jq = $(html);
    if(type == "toast") {
        jq.find(".window-panel").width(message.length * 20).css("margin-left", -message.length * 20 / 2).css("margin-top", -172 / 2);
    } else {
        jq.find(".window-panel").width(mizhu.width).css("margin-left", -mizhu.width / 2).css("margin-top", -172 / 2 - 36);
    }
    if(valempty(title)) {
